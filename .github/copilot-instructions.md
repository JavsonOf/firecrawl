...
- Cloud `@copilot` remains the primary engine. If Copilot quota/credits/plan/entitlement prevents cloud execution, or the user explicitly requests `FORCE BYOK`, use the v1.2 BYOK runner on the SAME PR head branch via the temporary `.github/superpowers/byok-task.json` trigger.
...
- If the standard independent reviewer is unavailable, use a separate read-only `mode: "review"` BYOK task and post its result as the PR review artifact/comment.
